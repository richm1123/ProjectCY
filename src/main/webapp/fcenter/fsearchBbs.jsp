...
// 페이지 10페이지가 안될 경우
  ...
  <div id="page_control">
	...
	%>
		<a href="fsearchBbs.jsp?pageNum=<%=startPage-pageBlock%>&searchText=<%=search%>&keyWord=<%=keyWord%>">Prev</a>
	<%
		}
		for(int i = startPage; i <= endPage; i++){
	%>
		<a href="fsearchBbs.jsp?pageNum=<%=i%>&searchText=<%=search%>&keyWord=<%=keyWord%>"><%=i %></a>
	
	<%
		}
		if(endPage < pageCount){
	%>
		<a href="fsearchBbs.jsp?pageNum=<%=startPage+pageBlock%>&searchText=<%=search%>&keyWord=<%=keyWord%>">Next</a>
	<%
		}
	%>
