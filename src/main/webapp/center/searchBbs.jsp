...
// List boardList = getBoardList()메서드 호출
  ...
  <article style="overflow-x: auto;">
   ...
   for(int i = 0; i < boardList.size(); i++){
	 ...
	 if(id!=null){  // 아이디 널값이 아닐경우 아이디값을 비교 
	   if(id.equals(boardDTO.getName())){ //일치하면 본인글 읽을수있음
	%>
   ...
   <form method="post" name="search" action="searchBbs.jsp">
	 <select name="keyWord">
	  ...
	  <option value="name">아이디</option>
		...
		<input type="text" name="searchText1" class="input_box">
  ...
  <div id="table_search">
		...
		<input type="button" value="목록으로" id="btn" class="btn" 
...
// 		int count=boardList.size(); // 검색해온 글 전체 갯수를 넣음
...
// 페이지 10페이지가 안될 경우
  ...
  <div id="page_control">
	...
	%>
		<a href="searchBbs.jsp?pageNum=<%=startPage-pageBlock%>&searchText=<%=search%>&keyWord=<%=keyWord%>">Prev</a>
	<%
		}
		for(int i = startPage; i <= endPage; i++){
	%>
		<a href="searchBbs.jsp?pageNum=<%=i%>&searchText=<%=search%>&keyWord=<%=keyWord%>"><%=i %></a>
	
	<%
		}
		if(endPage < pageCount){
	%>
		<a href="searchBbs.jsp?pageNum=<%=startPage+pageBlock%>&searchText=<%=search%>&keyWord=<%=keyWord%>">Next</a>
	<%
		}
	%>
